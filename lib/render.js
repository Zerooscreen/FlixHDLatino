...
function head({ title, description, url, image, robots, type }) {
  ...
  if (title) {
    const cleanTitle = title.replace(/\s*\|\s*FlixHDLatino/g, '').trim();
  ...
  return `
    ...
    <!-- Monetag Push Notification (Ganti 11565738 dengan Zone ID Anda jika perlu) -->
    <script src="https://alwingulla.com/88/tag.min.js" data-zone="11565738" async data-cfasync="false"></script>
  `;
}
  ...
  <body>
    <header>
      <div class="header-inner">
        <a class="logo" href="/">Flix<span>HD</span>Latino</span></a>
        <nav class="tabs">
